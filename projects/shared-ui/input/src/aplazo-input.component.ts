    ...
    <div class="relative">
      <div class="relative">
        ...
        <input
          ...
          class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 bg-white"
        />
        } @else {
        <input
          [id]="id"
      ...
      <label
        [for]="id"
        class="absolute bg-white px-2 left-3 top-0 transform -translate-y-1/2 text-sm font-medium text-dark"
      >
        {{ label }}
      </label>
      <span
        *ngIf="error"
        class="absolute text-sm text-alert-critical-icon bottom-[-20px]"
        >{{ error }}</span
      >
    </div>
  `,
...
export class AplazoInputComponent {
  ...
  @Input() dateFormats: string[] = ['dd/MM/yy'];
  ...
  onInput(event: Event) {
    ...
    this.value = value || null; // Manejar string vacío como null

    if (this.type === 'date') {
      this.onChange(value ? new Date(value).toISOString() : null);
    } else {
  ...
  writeValue(value: string): void {
    ...
    if (this.type === 'date') {
      // Force DOM update for date input
      setTimeout(() => {
